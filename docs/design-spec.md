...
### `usecases/collect.py` — 전체 수집 (init_scrap)

```
collect_all_jobs(scraper, store) -> CollectResult:
  1. store.clear()                        # DB 초기화
  2. total = scraper.get_total_count()
  3. total_pages = ceil(total / 50)
  4. for page in 1..total_pages:
       ids = scraper.fetch_listing_page(page)
       for id in ids:
         job = scraper.fetch_job_detail(id)
         if job: store.add_job(job)
       print progress
  5. return CollectResult(total, collected, failed)
```

### `usecases/sync.py` — 증분 동기화 (sync_scrap)

```
sync_jobs(scraper, store) -> SyncResult:
  ...
  3. for page in 1..total_pages:
       ...
       for id in new_ids:
         ...
         if job:
           ...
           new_count += 1
       print progress
  4. return SyncResult(scanned_pages, new_count, failed)
```

- 전체 목록 스캔, DB에 없는 공고만 상세 요청
- init_scrap 중단 후 실행 시 빠진 공고 자동 메움
