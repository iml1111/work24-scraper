...
고용24 목록 페이지는 여러 출처(워크넷/지자체/산업인력공단/인터넷)의 공고를 혼합 반환. 각 공고의 `infoTypeCd`와 `infoTypeGroup`이 다르며, `JobRef` dataclass로 ID+파라미터를 묶어 전달.

### 실패 분류 로깅

`fetch_job_detail`은 `tuple[Job | None, str]`을 반환 (status: `ok`/`expired`/`blocked`/`error`). 건별 한 줄 로그 + 페이지 요약 출력.
